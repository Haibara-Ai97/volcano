...
func getMemoryUsage(cgroupRoot string, cgroupVersion string) (int64, error) {
	...
	// Choose the correct memory usage file based on cgroup version
	...
	for _, line := range lines {
		...
		if cgroupVersion == cgroup.CgroupV2 {
			// For cgroup v2, calculate memory usage based on key memory metrics
			// Similar to cgroup v1's total_cache + total_rss + total_swap
			...
			// For cgroup v1, use original field names
